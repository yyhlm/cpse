...
from .code_release import build_code_release
...
from .optimizer import GepaPromptOptimizer, MiproV2InstructionOptimizer, MiproV2Optimizer, OproPromptOptimizer, TextGradPromptOptimizer
...
def main() -> int:
    ...
    if args.preflight:
        ...
        return 0
    special_mode_count = sum((args.report_only, args.retry_gold_audit, args.retry_failed_primary, bool(args.judge_only), args.metrics_only, args.statistics_only, args.usage_only, args.artifact_release, args.code_release, args.leakage_audit, args.blind_baseline_only))
    ...
    if args.code_release:
        print(build_code_release(
            ...
            output_dir=config.output_root.parent.parent / "code_release",
        ))
        return 0
    ...
    if args.judge_only:
        ...
        print(run_judge_only(config, args.run_id, judge, label=args.judge_only, force=args.judge_force))
        return 0
    if args.blind_baseline_only:
        ...
        judge = GoldJudge(
            ...
            include_error_locations=config.include_error_locations,
            include_pdf=config.include_pdf,
            include_schema=config.optimization_mode != "schema_free_direct",
        )
        runner = ExperimentRunner(config, extractor=extractor, judge=judge, auditor=None, optimizer_factory=None)
        print(runner.run_blind_baseline_only(args.run_id, invocation_command=_invocation_command()))
        return 0
    ...
    elif config.optimization_mode in {"two_stage_alternating_schema_description", "two_stage_coverage_plan_schema_description", "two_stage_evidence_routing_schema_description"}:
